...
%% Depict initial RV endo mesh
%{
figure;
hold on; axis equal; xlabel('x'); ylabel('y'); zlabel('z'); grid on;
trisurf([mesh.tri_n1' mesh.tri_n2' mesh.tri_n3'], mesh.node_x, mesh.node_y, mesh.node_z);
% patch('faces',[mesh.tri_n1' mesh.tri_n2' mesh.tri_n3'], 'vertices',[mesh.node_x, mesh.node_y, mesh.node_z], 'FaceColor', 'w', 'FaceAlpha', 0.5);
for slice=1:size(self.hShowMesh.rois,1)
...
%% Export figures:
% 1. set breakpoint @ "meshes.vertices = Adjust_Mesh_Mex ..."
% 2. run codes "IOP = zeros(1,0); ... view(nor);"
% 3. run codes below
% tmp = get(self.configObj, 'Location.LoadWorkspace', pwd);
...
scatter3(center(:,1),center(:,2),center(:,3),400,'k','*','LineWidth',3,'MarkerFaceColor', 'r');
scatter3(center(:,1)-4,center(:,2),center(:,3),400,'k','x','LineWidth',4,'MarkerFaceColor', 'b');
% h = impoint(gca,center(1:2));
% fcn = makeConstrainToRectFcn('impoint',get(gca,'XLim'),get(gca,'YLim'));
% setString(h, 'Interactive Seed Point');
% setPositionConstraintFcn(h,fcn);
% setColor(h,'r');
% addNewPositionCallback(h,@(pos) proj(pos));
%}
...
% hgexport(gcf,fullfile(tmp,[num2str(gcf),sprintf('_%.1g',[initial_nor_R,final_nor_R]),'.jpg']),hgexport('factorystyle'), 'Format', 'jpeg');%R2014b or above:get(gcf,'Number')
% hgsave(gcf,fullfile(tmp,[num2str(gcf),sprintf('_%.1g',[initial_nor_R,final_nor_R]),'.fig']));
% hgexport(,'.jpg',hgexport('factorystyle'), 'Format', 'jpeg');%R2014b or above:get(gcf,'Number')
% hgsave(,'.fig');
					end
				end
				...
				for final_tan_R = 0.1:0.1:1
					for initial_tan_R = 0.1:0.1:final_tan_R
						...
						title(['initial_nor_R=',num2str(initial_nor_R),';','final_nor_R=',num2str(final_nor_R),';','initial_tan_R=',num2str(initial_tan_R),';','final_tan_R=',num2str(final_tan_R)]);
						 %}
						patch(meshes, 'FaceColor', 'w', 'FaceAlpha', 0.5);
% trisurf(meshes.faces, meshes.vertices(:,1)', meshes.vertices(:,2)', meshes.vertices(:,3)'); % color is proportional to surface height
						% scatter3(points(:,1),points(:,2),points(:,3),3,'k','*');
						for slice=1:size(self.hShowMesh.rois,1)
							plot3(self.hShowMesh.rois{slice,3}(:,1),self.hShowMesh.rois{slice,3}(:,2),self.hShowMesh.rois{slice,3}(:,3),'LineWidth',1,'color', 'b');
						end
...
%% Adjustment with GUI
%{ 
[tmp.PANEL_COLOR, tmp.FIGURE_BACKGROUND_COLOR, tmp.GUI_BACKGROUND_COLOR] = deal([ 231 234 241 ] / 255);
tmp.TEXT_COLOR = [ 0 0 0 ] / 255;
meshes.vertices = Adjust_Mesh(mesh, points, tmp, 'RVendo');
%}
			meshes.vertices = Adjust_Mesh_Mex(mesh, points, iterations, [initial_smoothness_weight, final_smoothness_weight], [initial_points_weight, final_points_weight], [initial_nor_R, final_nor_R],  [initial_tan_R, final_tan_R]);
...
% Bug? dense3D_plugin: start from frame#0 instead of frame#1!
				...
				if idxEndo < 2
					...
					% results.CLShearAngle = rad2deg(self.dataObj.torsion(results));% CMD "asin" used in DENSE3D.m
					results.CLShearAngle = CLShearAngle(results,'deg');
...
%% NUMBER OF FRAME supported in 3D DENSE Analysis Viewer is self.hShowMesh.frame instead of self.hShowMesh.Frame(=self.hShowMesh.frame+1):
				...
				for k = 1:numel(fields)
					tmp = output.(fields{k});
                    if abs(size(tmp,2)-nFrames) < 1
%self.viewerObj.cache.regionalStrain.(fields{k})(:,1) = [];
                        self.viewerObj.cache.regionalStrain(1).(fields{k})(:,1) = [];
						if numel(output)>1; self.viewerObj.cache.regionalStrain(2).(fields{k})(:,1) = []; end
					end
				end
...
% self.viewerObj.Frames = nFrames;
				...
				if strcmpi(button,'Yes')				
					...
					DENSE3Dobject.Mesh.circInd = Cseg;
					DENSE3Dobject.Mesh.longInd = Lseg;
					Strains.CLShearAngle = CLShearAngle(Strains,'deg');
					DENSE3Dobject.Mesh.strains = Strains;
					% DENSE3Dobject.Mesh.strains.torsion = rad2deg(self.dataObj.torsion(Strains));
...
% Last Modified: 17:28 August 7, 2018
% Modified By: Zhan-Qiu Liu (lafeir.lew@gmail.com)
	if size(structure(1).(fieldName),1) < 2 % == 1
...
function result = CLShearAngle(strains, unit)
% strains - strain tensor
% unit - 'deg' or 'rad'
	if strncmpi(unit,'deg',3) 
		result = -asind(2 * strains.CL ./ sqrt((1 + 2 * strains.CC) .* (1 + 2 * strains.LL)));
	else
